...
function RestockCell({ item, onRestock }) {
  ...
  const inputId = `restock-qty-${item._id}`;
  ...
  const [form, setForm] = useState(
    editItem
      ? {
          ...
          minRequired:  editItem.minRequired,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/70 backdrop-blur-sm">
      <div className="bg-neutral-950 border border-neutral-800 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
        <div className="px-8 py-6 border-b border-neutral-800 flex items-center justify-between">
          <div>
            <h2 className="text-base font-black text-neutral-100">
              {editItem ? "Edit Inventory Item" : "Add Inventory Item"}
            </h2>
            <p className="text-[11px] text-neutral-500 mt-0.5 uppercase tracking-wider font-bold">
              {editItem ? `Editing: ${editItem.itemName}` : "New stock entry"}
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-5">
          <div>
            <label htmlFor="inv-itemName" className={labelCls}>Item Name</label>
            <input
              id="inv-itemName"
              required
              name="itemName"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="inv-currentStock" className={labelCls}>Current Stock</label>
              <input
                id="inv-currentStock"
                required
                type="number"
            ...
            <div>
              <label htmlFor="inv-minRequired" className={labelCls}>Min. Required</label>
              <input
                id="inv-minRequired"
                required
                type="number"
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 py-3 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-neutral-300 font-bold text-sm transition-all">
              Cancel
            </button>
...
export default function InventoryManagement({ onStockChange, downloadReportRef }) {
  ...
  const exportToExcel = useCallback(() => {
    const rows = filteredItems.map((item, index) => ({
      ...
      "Current Stock": typeof item.currentStock === "number" ? item.currentStock : "—",
      "Min. Required": typeof item.minRequired  === "number" ? item.minRequired  : "—",
      "Unit":          item.unit || "—",
      "Status":
        item.currentStock === 0
          ? "Out of Stock"
          : item.currentStock <= (item.minRequired || 0)
          ? "Critical"
          : "Optimal",
    }));
    ...
    XLSX.writeFile(wb, `SouthernTales_Inventory_${new Date().toISOString().slice(0, 10)}.xlsx`);
  }, [filteredItems]);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <button
          onClick={handleAdd}
          className="bg-gradient-to-r from-orange-400 to-orange-600 text-black font-black text-sm px-4 py-2 rounded-lg hover:shadow-lg hover:shadow-orange-900/40 transition-all"
        >
          + Add Item
        </button>
      </div>
                    ...
                    <tr key={item._id || index} className="group hover:bg-neutral-900/50 transition-all duration-200">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="font-bold text-sm text-neutral-100 group-hover:text-orange-400 transition-colors">
                            {item.itemName}
                          </span>
                      ...
                      <td className="px-6 py-4 text-center">
                        ...
                        {isOutOfStock && (
                          <RestockCell
                            item={item}
                            onRestock={async (i, q) => { await restockItem(i, q); onStockChange?.(); }}
                          />
                        )}
                      </td>
                      <td className="px-6 py-4 text-center">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            ...
                            title="Edit item"
                            className="p-2 bg-neutral-800 hover:bg-orange-500 text-neutral-400 hover:text-black rounded-lg transition-all duration-200"
                          >
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 256 256">
