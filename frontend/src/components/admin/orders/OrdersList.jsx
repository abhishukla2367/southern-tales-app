...
<div>
              ...
              <input type="text" placeholder="e.g. Rahul Sharma" value={guestName}
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <button type="button" onClick={refreshTables} disabled={tablesLoading}
                  className="text-[10px] text-[#f5c27a] hover:underline disabled:opacity-40 transition">
                  {tablesLoading ? "Refreshing..." : "↻ Refresh"}
                </button>
              </div>
              {tablesError && (
                <p className="text-[10px] text-amber-400 mb-1.5">⚠ Could not load live table data. Showing default T1–T20.</p>
              )}
                  ...
                  {availableTables.length > 0 && (
                    <optgroup label="✅ Available">
                      {availableTables.map((t) => (
                        <option key={t.tableNumber} value={t.tableNumber} className="bg-[#111] text-white">
                  ...
                  {occupiedTables.length > 0 && (
                    <optgroup label="🔴 Occupied">
                      {occupiedTables.map((t) => (
                        <option key={t.tableNumber} value={t.tableNumber} disabled className="bg-[#111] text-[#555]">
              ...
              <div className="relative">
                <select value={numGuests} onChange={(e) => setNumGuests(e.target.value)}
                  ...
                  {Array.from({ length: 8 }, (_, i) => i + 1).map((n) => (
                    <option key={n} value={n} className="bg-[#111] text-white">{n} {n === 1 ? "Guest" : "Guests"}</option>
                  ))}
                </select>
                <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[#555]">▾</span>
              </div>
          ...
          <div className="flex flex-col" style={{ maxHeight: "75vh" }}>
            ...
            <div className="px-6 py-4 border-t border-[#1f1f1f] bg-[#111]">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs text-[#aaa]">{selectedItems.reduce((s, i) => s + i.quantity, 0)} item(s) selected</span>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={handlePlaceOrder} disabled={placing || selectedItems.length === 0}
    ...
    <>
                  ...
                  return (
                    <tr key={order._id} className="border-b border-[#1a1a1a] hover:bg-[#161616] transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md border ${typeClass}`}>{typeLabel}</span>
                      </td>
                      <td className="px-6 py-4">
                        <p className="text-sm font-bold text-[#f1f1f1]">
                          {isInHouse ? (order.guestName || "Walk-in Guest") : (order.userId?.name || "Guest")}
                        </p>
                        {isInHouse ? (
      ...
      {showHoursConfirm && (
        <OutOfHoursConfirm
          ...
          onProceed={() => { setShowHoursConfirm(false); setShowWalkInModal(true); }}
        />
      )}
